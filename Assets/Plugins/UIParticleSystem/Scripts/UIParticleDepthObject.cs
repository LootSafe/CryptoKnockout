...
public class UIParticleDepthObject : MonoBehaviour
{
    ...
    private bool GetMeshAndTextureFromMaskTexture(ref Mesh mesh, out Texture2D texture)
    {
        ...
        int[] indices;
        Vector2[] uvs;
        if(!GenerateMeshDataFromRectTransform(rectTrans, out vertices, out indices, out uvs))
            return false;
    ...
    private bool GenerateMeshDataFromRectTransform(RectTransform rectTrans, out Vector3[] vertices, out int[] indices, out Vector2[] uvs)
    {
